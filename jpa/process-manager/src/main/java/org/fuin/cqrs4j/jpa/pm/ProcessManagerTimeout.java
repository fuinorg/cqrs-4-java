 ...
 * expected. Rows are inserted/updated within the process manager view's transaction (via the process-timeout
 * service) and swept by the process-timeout sweeper; a row that passes its deadline without being cancelled is
 * handed to the application's timeout handler. The primary key is the process instance id, so there is one
 * pending timeout per process.
 */
@NotThreadSafe
@Entity
@Table(name = "CQRS4J_PM_TIMEOUT")
