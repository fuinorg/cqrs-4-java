...
## 0.7.0
- **The Spring Boot and Quarkus flavours moved to their own repositories**, [cqrs-4-springboot](https://github.com/fuinorg/cqrs-4-springboot) and [cqrs-4-quarkus](https://github.com/fuinorg/cqrs-4-quarkus). They were the bulk of this repository and changed on their own cadence. **No coordinates changed**: `org.fuin.cqrs4j:cqrs-4-java-springboot-security` and friends keep their groupId and artifactId. What a consumer must change is one line - each new repository publishes its own BOM (`cqrs-4-java-springboot-bom`, `cqrs-4-java-quarkus-bom`) to import alongside `cqrs-4-java-bom`, which now manages only what this repository builds. Keeping them apart means no BOM can name a version of something released on another cadence.
- **New module `cqrs-4-java-springboot-security`**: the `SecurityFilterChain` an application serves its API behind, built from `cqrs4j.security.*` properties. Adding the dependency and setting `issuer-uri`/`audiences` is enough - stateless, CSRF off, health and info open, everything else authenticated - and role rules are YAML. It carries the three things both existing consumers got wrong independently: ordering the chain `before` *both* Boot auto-configurations, handing `KeycloakJwtAuthenticationConverter` to `jwt(...)` explicitly, and permitting the actuator again after `ManagementWebSecurityAutoConfiguration` backs off. Deliberately **not** part of the keycloak starter, which validates tokens and needs no `spring-security-config`.
  - The property model has no way to open a path. Rules require roles; the only permit is a named actuator toggle. An application needing a public path declares a `Customizer` bean - in Java, where `SecurityArchRules` can see it. Putting that in YAML would defeat every guard there is.
- `cqrs-4-java-test-helper` gained `SecurityArchRules` (a permit-all chain may exist only under `@Profile("local")`, plus the packaged `application.yml` may not activate that profile) and `KeycloakRealm` + `TestHelper.createKeycloakContainer` (a provisioned realm and real tokens, for the cases a stub cannot show - notably a correctly signed token issued for another audience). ArchUnit, snakeyaml and AssertJ are `optional` there, so a consumer using only the container helpers does not inherit them.
