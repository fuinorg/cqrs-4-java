...
/**
 * A command that could not be delivered to the command endpoint after exhausting all retries. It is
 * moved here by the {@link CommandQueueExecutor} so that it no longer blocks the outbox queue while
 * remaining available for manual inspection and re-processing.
 */
@NotThreadSafe
@Entity
@Table(name = "SPRING_PM_CMD_DEAD_LETTER")
