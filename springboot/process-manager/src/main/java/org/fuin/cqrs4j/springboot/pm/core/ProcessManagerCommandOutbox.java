...
/**
 * A command produced by a process manager that is waiting to be dispatched to the command endpoint.
 * Rows are inserted within the same transaction as the process manager's state change (transactional
 * outbox) and drained asynchronously by the {@link CommandQueueExecutor}.
 */
@NotThreadSafe
@Entity
@Table(name = "SPRING_PM_CMD_OUTBOX")
